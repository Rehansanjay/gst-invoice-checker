 ...
 *
 * Must be referenced explicitly by any page that declares its own `openGraph`
 * block: doing so suppresses the file-convention image that pages without one
 * inherit automatically, which silently left /bulk, /guides and every
 * error-code page with no preview image at all.
 */
